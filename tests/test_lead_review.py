...
def _assert_layer_missing(output: str, layer_name: str) -> None:
    """Verify that a layer renders compactly as 'layer_name: missing'."""
    pattern = rf"^{re.escape(layer_name)}: missing"
    assert re.search(pattern, output, re.MULTILINE), f"{layer_name}: missing not found in output"
...
def test_render_graphql_payload_fills_layer_fields() -> None:
    ...
    lead["raw_payload_json"] = json.dumps({
        ...
        "_marketplace_raw": {
            "skills": [{"prettyName": "WordPress"}, {"prettyName": "WooCommerce"}, {"name": "PHP"}],
            "client": {
                "verificationStatus": "VERIFIED",
                "location": {"country": "United States"},
                "totalSpent": {"displayValue": "$10K"},
                "totalHires": 12,
                "totalPostedJobs": 20,
            }
        }
    ...
    _assert_face_value_field(mp_section, "Skills:", "WordPress, WooCommerce, PHP")
    # Public section
    ...
    _assert_face_value_field(pub_section, "Proposals:", "10")
...
def test_render_graphql_fixed_price_budget() -> None:
    ...
    assert "graphql_layer" not in output
    pub_section = _layer_section(output, "public_search_layer", "by_id_layer")
    _assert_face_value_field(pub_section, "Contract:", "fixed")
    _assert_face_value_field(pub_section, "Budget:", "$500")
    _assert_face_value_field(pub_section, "Hourly range:", ".")
...
def test_render_graphql_invalid_json_is_safe() -> None:
    ...
    # Initial face-value section still has proposals
    assert "Proposals:   20 to 50" in output
    # Marketplace and public layers have missing fields
    _assert_layer_missing(output, "marketplace_search_layer")
    _assert_layer_missing(output, "public_search_layer")
    # Unsupported labels absent from public layer (they should not be present at all now)
...
def test_render_split_face_value_sections_source_vs_exact() -> None:
    ...
    # Verify both new source sections exist
    assert "marketplace_search_layer" in output
    assert "public_search_layer" in output
    ...
    _assert_face_value_field(pub_section, "Contract:", "hourly")
    assert "Hires:" not in pub_section
    assert "Persons to hire:" not in pub_section
...
def test_marketplace_layer_reads_from_marketplace_raw() -> None:
    ...
    assert "by_id_layer" in by_id_section
    assert "manual_scrape_layer: missing" in output
...
def test_render_graphql_lead_includes_manual_scrape_layer() -> None:
    ...
    # Check that manual_scrape_layer is present (as missing)
    assert "manual_scrape_layer" in output
    assert "manual_scrape_layer: missing" in output
...
def test_render_manual_scrape_layer_missing_collapses() -> None:
    lead = _make_lead()
    output = render_raw_lead_review(lead)
    
    assert "manual_scrape_layer: missing" in output
    assert "Manual title:" not in output
    assert "Raw manual text:" not in output


def test_render_manual_scrape_layer_with_text_does_not_collapse() -> None:
    lead = _make_lead()
    lead["manual_scrape_raw_manual_text"] = "some manual text"
    output = render_raw_lead_review(lead)
    
    assert "manual_scrape_layer" in output
    assert "manual_scrape_layer: missing" not in output
    assert "Raw manual text:     some manual text" in output


def test_render_manual_scrape_layer_with_parsed_fields_does_not_collapse() -> None:
    lead = _make_lead()
