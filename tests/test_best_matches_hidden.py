...
def test_import_skips_hidden_feedback(tmp_path, monkeypatch):
    db_path = tmp_path / "test.db"
    conn = connect_db(db_path)
    initialize_db(conn)

    html = """
    <section class="air3-card-section" data-ev-opening_uid="123">
        <a href="/jobs/normal_~02123/">Normal Job</a>
    </section>
    <section class="air3-card-section" data-ev-opening_uid="456">
        <a href="/jobs/hidden_~02456/">Hidden Job</a>
    ...
    summary = import_best_matches_html(conn, html, config=object())
...
def test_cli_import_reports_hidden_feedback(tmp_path, monkeypatch):
    ...
    assert "Exact hydration: success=1 failed=0 skipped=0" in out
